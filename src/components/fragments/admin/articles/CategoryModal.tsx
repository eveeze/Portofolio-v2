...
const CategoryModal: React.FC<CategoryModalProps> = ({
  ...
  const handleSubmit = async () => {
    if (!formData.name.trim() || !formData.slug.trim() || !token) return;
    ...
    try {
      ...
      if (editingId) {
        ...
        response = await fetch(
          `${apiBaseUrl}/articles/categories/${editingId}`,
          {
            method: "PATCH",
            headers: {
        ...
        // Create new category
        response = await fetch(`${apiBaseUrl}/articles/categories`, {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error ||
            `Failed to ${editingId ? "update" : "create"} category: ${response.status}`
        );
      }
      ...
      alert(
        `Failed to ${editingId ? "update" : "create"} category: ${error.message}`
      );
    } finally {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const apiBaseUrl = getApiBaseUrl();
      const response = await fetch(`${apiBaseUrl}/articles/categories/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
                      ...
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          <button
                            ...
                            disabled={isDeleting}
                            className="p-2 text-grayText hover:text-red-400 hover:bg-red-900/20 rounded-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                            title="Delete"
                          >
                            {isDeleting ? (
                              <div className="w-4 h-4 border-2 border-grayText border-t-transparent rounded-full animate-spin" />
                            ) : (
                              <svg
                                className="w-4 h-4"
                                fill="none"
