...
const EditArticle: React.FC = () => {
  ...
  const currentArticle = articles?.find((a) => a._id === id);

  // Transform tech stacks - Fix: Handle null values properly
  const techStacks =
  ...
  useEffect(() => {
    if (currentArticle) {
      setFormData({
        ...
        // Fix: Filter out null values and map to IDs
        tags: currentArticle.tags
          .filter((tag) => tag !== null && tag !== undefined)
          .map((tag) => (typeof tag === "string" ? tag : tag._id)),
        metaDescription: currentArticle.metaDescription || "",
        featuredImage: null,
        featuredImageUrl: currentArticle.featuredImage,
        status: currentArticle.status,
        publishedAt: currentArticle.publishedAt,
        scheduledAt: currentArticle.scheduledAt,
        readingTime: currentArticle.readingTime,
        // Fix: Handle techStack properly
        techStack:
          currentArticle.techStack
            ?.filter((tech) => tech !== null && tech !== undefined)
            .map((tech) => (typeof tech === "string" ? tech : tech._id)) || [],
        images: [],
        existingImages: currentArticle.images || [],
        imagesToDelete: [],
        seriesId: currentArticle.series?._id || "",
        seriesPosition: currentArticle.series?.position || 1,
      });

      // Set scheduled state if article has scheduledAt
      if (currentArticle.scheduledAt) {
        setIsScheduled(true);
        const scheduledDateTime = new Date(currentArticle.scheduledAt);
  ...
  useEffect(() => {
    if (!isInitialLoading && !currentArticle && articles) {
      navigate("/admin/articles");
    }
  }, [isInitialLoading, currentArticle, articles, navigate]);
  ...
  const handleSubmit = async (status: "draft" | "published" | "archived") => {
    ...
    try {
      ...
      } else if (status === "draft" && !isScheduled) {
        // Clear scheduledAt if switching to draft without scheduling
  ...
  const saveArticle = async (data: ArticleFormData) => {
    if (!token || !id) {
      throw new Error("No authentication token or article ID available");
    }

    const apiBaseUrl = getApiBaseUrl();

    // Handle existing image deletions first
    ...
    if (data.seriesId && data.seriesPosition !== undefined) {
      // First remove from existing series
      await fetch(`${apiBaseUrl}/articles/series-articles/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
      }).catch(() => {}); // Ignore error if not in series

      // Then add to new series
    ...
    } else if (currentArticle?.series) {
      // Remove from series if no longer selected
      await fetch(`${apiBaseUrl}/articles/series-articles/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const isLoadingData =
    !categories || !tags || !series || !techStacks || isInitialLoading;
            ...
            <div className="flex items-center space-x-3">
              ...
              {formData.status === "published" && (
                <button
                  ...
                  disabled={isLoading}
                  className="px-6 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-lg font-medium transition-all duration-200 disabled:opacity-50"
                >
                  Archive
                </button>
              )}
                        ...
                        {formData.existingImages.map((image) => (
                          <div key={image._id} className="relative group">
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                handleRemoveExistingImage(image._id)
                              }
